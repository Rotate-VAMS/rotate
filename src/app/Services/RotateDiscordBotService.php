...
class RotateDiscordBotService
{
    public function handle(Message $message): void
    {
        ...
        // Handle event PIREP command
        if ($content === '!rotate pe' && !$message->channel->is_private) {
            $user = User::where('discord_id', $userId)->first();
            if (!$user) {
                $message->author->sendMessage("❌ Your Discord account is not linked to a Rotate account. Please link your account first.");
                return;
            }
            $message->author->sendMessage("🎯 Let's start filing your Event PIREP.\nFetching past events...");
            RotateDiscordBotSessionManager::start($userId, 'event_pirep');
            // Initialize the session with fetch_events step and immediately send event list
            RotateDiscordBotSessionManager::update($userId, '', '', 'fetch_events');
            $this->handleEventPirepSession($message, $userId, '');
            return;
        }

        // Handle DM step-by-step session
        if ($message->channel->is_private && RotateDiscordBotSessionManager::exists($userId)) {
            $sessionType = RotateDiscordBotSessionManager::getSessionType($userId);
            if ($sessionType === 'event_pirep') {
                $this->handleEventPirepSession($message, $userId, $content);
            } else {
    ...
    protected function handleEventPirepSession(Message $message, string $userId, string $content): void
    {
        $user = User::where('discord_id', $userId)->first();
        if (!$user) {
            $message->channel->sendMessage("❌ Your Discord account is not linked to a Rotate account. Please link your account first.");
        ...
        switch ($step) {
            case 'fetch_events':
                // Fetch past events
                $pastEvents = Event::withoutGlobalScope('tenant')
                ...
                if ($pastEvents->isEmpty()) {
                    $message->author->sendMessage("❌ No past events found. Session cancelled.");
                    RotateDiscordBotSessionManager::cancel($userId);
                    return;
                }
                
                $eventList = "Please select an event by number:\n";
                $eventIds = [];
                foreach ($pastEvents as $i => $event) {
                    $eventDate = date('Y-m-d H:i', $event->event_date_time);
                    $eventList .= ($i+1) . ". **{$event->event_name}** ({$event->origin} → {$event->destination}) - {$eventDate}\n";
                    $eventIds[] = (string)$event->id;
                }
                RotateDiscordBotSessionManager::update($userId, 'eligible_events', json_encode($eventIds), 'select_event');
                $message->author->sendMessage($eventList);
                break;

            case 'select_event':
                $eligibleEvents = json_decode($data['eligible_events'] ?? '[]', true);
                $eventIndex = (int)$content - 1;
                if (!isset($eligibleEvents[$eventIndex])) {
                    $message->channel->sendMessage("❌ Invalid selection. Please enter a valid event number.");
                    return;
                }
                $eventId = $eligibleEvents[$eventIndex];
                
                // Check if user was a participant in this event
                $wasParticipant = \App\Models\Event::checkIsUserWasParticipant($eventId, $user->id);
                if (!$wasParticipant) {
                    $message->channel->sendMessage("❌ You were not registered for this event. Only event participants can file event PIREPs. Session cancelled.");
                    RotateDiscordBotSessionManager::cancel($userId);
                    return;
                }
                
                RotateDiscordBotSessionManager::update($userId, 'event_id', $eventId, 'hours');
                $message->channel->sendMessage("✈️ Great.\nEnter **flight time hours** (e.g., 2):");
                break;
            ...
            case 'flight_type':
                ...
                if (empty($requiredFields)) {
                    RotateDiscordBotSessionManager::update($userId, 'custom_fields', json_encode([]), 'complete');
                    $this->finalizeEventPirep($message, $userId);
                    return;
                }
                RotateDiscordBotSessionManager::update($userId, 'custom_fields', json_encode($requiredFields), 'custom_field_0');
                $this->promptCustomField($message, $userId, 0);
    ...
    protected function promptCustomField(Message $message, string $userId, int $index): void
    {
        ...
        if (!isset($customFields[$index])) {
            ...
            // Check session type to call appropriate finalization method
            $sessionType = RotateDiscordBotSessionManager::getSessionType($userId);
            if ($sessionType === 'event_pirep') {
                $this->finalizeEventPirep($message, $userId);
            } else {
    ...
    protected function finalizeEventPirep(Message $message, string $userId): void
    {
        $data = RotateDiscordBotSessionManager::getData($userId);
        $user = User::where('discord_id', $userId)->first();
        if (!$user) {
        ...
        $result = \App\Models\Pirep::createEditPirep($pirepData, 'create', true);
        if (isset($result['error'])) {
            $message->channel->sendMessage("❌ Failed to save Event PIREP: " . $result['error']);
        } else {
            $message->channel->sendMessage("✅ Your Event PIREP has been recorded successfully!");
        }
        RotateDiscordBotSessionManager::cancel($userId);
    }
}