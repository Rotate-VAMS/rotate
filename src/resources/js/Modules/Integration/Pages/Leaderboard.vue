        ...
        <div class="space-y-6">
            ...
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 sm:mb-6 gap-3 sm:gap-0">
                <div class="w-full sm:w-auto">
                    ...
                    <p class="text-xs sm:text-sm text-gray-500 mt-1">View the top performing pilots ranked by activity and performance</p>
                </div>
            </div>
            <div class="bg-white rounded-xl shadow-xl border border-gray-200 p-4 sm:p-6 glassmorphism">
              ...
              <div v-else-if="leaderboardData.length > 0" class="space-y-3 mb-6">
                <div 
                  v-for="(pilot, index) in filteredLeaderboardData" 
                  :key="pilot.id"
                  :class="[
...
const filteredLeaderboardData = computed(() => {
  ...
  return leaderboardData.value.filter(pilot => 
    pilot.user_name.toLowerCase().includes(searchTerm) ||
    (pilot.callsign && pilot.callsign.toLowerCase().includes(searchTerm))
  );
});
...
const getTierBadgeClass = (index) => {
  ...
  return 'bg-gray-100 text-gray-800 border border-gray-300 shadow-sm font-semibold'
}
